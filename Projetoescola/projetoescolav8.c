...
int main() {
  ...
  int opcao, i, j, k, cont = 0, opcaoaluno, opcaoprof, opcaodisc, opcaodiversa,
                      ...
                      ln, cadastro, matricula, alunocadastrado, alunocad,
                      codigo, achou = 0,
    ...
    switch (opcao) {
      ...
      case 1: {
        ...
        case 1: {
          ...
          if (qtdalunos == TAMALUNO) { // teste para bloquear a inserção de
            ...
            printf("\n");
            if (qtdalunos >
                0) { // procedimento para ordenar os alunos por ordem alfabetica
              setbuf(stdin, 0);
      ...
      case 2: {
        ...
        case 1: { // cadastro de professores
          if (qtdprof == TAMPROF) {
            ...
            if (qtdprof > 0) { // procedimento para ordenar os professores por
                               // ordem alfabetica
              setbuf(stdin, 0);
              for (i = qtdprof; i > 0; i--) {
                copia2 = prof[i - 1];
        ...
        case 2: {
          ...
          if (qtdprof == 0) { // teste para verificar se a matrícula existe
            ...
            ln = 1;
            for (i = 0; i < qtdprof;
                 i++) { // listagem dos professores em ordem alfabetica
              if (prof[i].ativo == 1) {
                printf("Professor(a) %d:\n", ln);
                printf("Nome: %s\n", prof[i].nome);
                printf("Matricula: %d\n", prof[i].matricula);
          ...
          if (qtdprof == 0) {
            ...
            for (i = 0; i < qtdprof;
              ...
              if (matricula == prof[i].matricula) {
                ...
                if (qtdprof > 0) { // procedimento para ordenar os professores
        ...
        case 5: {
          if (qtdprof == 0) {
            ...
            if (sexo == 'M') {
              ...
              if (achou == 0) {
                printf("Nao foi possivel encontrar nenhum professor do sexo "
                       "informado.\n");
                break;
              }
              break;
            ...
            for (i = 0; i < qtddisc; i++) {
              if (codigo == disciplinas[i].codigo) {
                ...
                for (ln = 0; ln < qtdalunos; ln++) {
                  ...
                  if (achou > 1) {
                    strcpy(
                        disciplinas[i].alunodisc[disciplinas[i].alunocad].cpf,
                        alunos[ln].cpf);
                    strcpy(
                        disciplinas[i].alunodisc[disciplinas[i].alunocad].nome,
                        alunos[ln].nome);
