...
int menualuno(opcaoaluno);
int menuprof(opcaoprof);
int menudisciplina(opcaodisc);
...
int main() {
  ...
  int opcao, i, opcaoaluno, opcaoprof, opcaodisc,
      dataerrada = 1, qtdalunos = 0, qtdprof = 0, qtddisc = 0, ln,
      matricula; // declaração de váriaveis auxiliares
  ...
  do {
      ...
      case 1: // cadastro de disciplinas
        ...
        do {
          ...
          scanf("%d%d%d", &prof[i].nasc.dia, &prof[i].nasc.mes,
                &prof[i].nasc.ano);
          if (prof[i].nasc.dia > 31 || prof[i].nasc.mes > 12 ||
              prof[i].nasc.ano > 2023 || prof[i].nasc.ano < 1920)
            dataerrada++;
          printf("\n");
        } while (dataerrada > 0);
        qtdprof++;
    ...
    case 3:
      do {
        ...
        case 3: {
          // listagem das discipinas if(qtddisc == 0)
          if (qtddisc == 0)
